...
-(void)createTableFooter{
    ...
    else if ([dataArray isEqual:wuxinghaohua ]){
        
    }
    else if ([dataArray isEqual:sixinggaodang ]){
        
    }
    else if ([dataArray isEqual:sanxingshushi ]){
        
    }
    else if ([dataArray isEqual:jijingliansuo ]){
        
    }
    else if ([dataArray isEqual:qingnianlvshe ]){
        
    }
    else if ([dataArray isEqual:tesebingguan ]){
        
    }
    else if ([dataArray isEqual:pingfenpaixu]){
        
    }
    else if ([dataArray isEqual:julipaixu ]){
        
    }
    else if ([dataArray isEqual:jiagecongdidaogao ]){
        
    }
    else if ([dataArray isEqual:jiageconggaodaodi ]){
...
- (void)viewDidLoad
{
    ...
    wuxinghaohua=[[NSMutableArray alloc]init];
    sixinggaodang=[[NSMutableArray alloc]init];
    sanxingshushi=[[NSMutableArray alloc]init];
    jijingliansuo=[[NSMutableArray alloc]init];
    qingnianlvshe=[[NSMutableArray alloc]init];
    tesebingguan=[[NSMutableArray alloc]init];
    pingfenpaixu=[[NSMutableArray alloc]init];
    julipaixu=[[NSMutableArray alloc]init];
    jiageconggaodaodi=[[NSMutableArray alloc]init];
    jiagecongdidaogao=[[NSMutableArray alloc]init];
    
    zongshuzu=[[NSMutableArray alloc]initWithObjects:arrayTotal,wuxinghaohua,sixinggaodang,sanxingshushi,jijingliansuo,qingnianlvshe,tesebingguan,pingfenpaixu,julipaixu,jiageconggaodaodi,jiagecongdidaogao, nil];
    ...
    NSArray *array1=[NSArray arrayWithObjects:@"LV_Class_hotel_",@"LV_Class_hotel_wu",@"LV_Class_hotel_si",@"LV_Class_hotel_san",@"LV_Class_hotel_jingji",@"LV_Class_hotel_qingnian",@"LV_Class_hotel_tese", nil];
    array2=[[NSArray alloc]initWithObjects:@"全部类型",@"五星/豪华",@"四星/高档",@"三星/舒适",@"经济连锁",@"青年旅社",@"特色宾馆",nil];
    ...
    NSArray *array3=[NSArray arrayWithObjects:@"LV_Class_order_",@"LV_Class_order_pingfen",@"LV_Class_order_juli",@"LV_Class_order_PriceUp",@"LV_Class_order_PriceDown",nil];
    array4=[[NSArray alloc]initWithObjects:@"默认排序",@"评分排序",@"距离排序",@"价格从低到高",@"价格从高到低",nil];
    ...
    NSArray *arraySelect=[NSArray arrayWithObjects:@"select * from listdo_hotel_main",@"select * from listdo_hotel_main where typename='五星/豪华'",@"select * from listdo_hotel_main where typename='四星/高档'",@"select * from listdo_hotel_main where typename='三星/舒适'",@"select * from listdo_hotel_main where typename='经济连锁'",@"select * from listdo_hotel_main where typename='青年旅舍'",@"select * from listdo_hotel_main where typename='特色宾馆'",@"select * from listdo_hotel_main order by score desc",@"select * from listdo_hotel_main order by distance desc",@"select * from listdo_hotel_main order by price desc",@"select * from listdo_hotel_main order by price asc", nil];
    ...
    for(int i=0;i<[arraySelect count];i++){
        ...
        while ([set next]) {
            HotelItem *cell=[[HotelItem alloc]init];
            cell.hotelname=[set objectForColumnName:@"hotelname"];
            cell.imageid=[set objectForColumnName:@"imageid"];
            cell.comment=[set objectForColumnName:@"comment"];
            cell.score=[[set objectForColumnName:@"score"]floatValue];
            cell.typename=[set objectForColumnName:@"typename"];
            cell.hotelid=[set objectForColumnName:@"hotelid"];
    ...
    for(int i=0;i<20;i++){
        ...
        HotelItem *cell=[dataArray objectAtIndex:i];
        MyAnnotation *annotation1=[[MyAnnotation alloc]initWithTitle:cell.hotelname subtitle:cell.comment location:CLLocationCoordinate2DMake([cell.jing floatValue],[cell.wei floatValue])];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
   ...
   HotelItem *item=[dataArray objectAtIndex:indexPath.row];
    UIView *backView=[[UIView alloc]initWithFrame:CGRectMake(0, 0, 320, 90)];
    UIImageView *img3=[[UIImageView alloc]initWithImage:[UIImage imageNamed:[NSString stringWithFormat:@"%@.jpg",item.imageid]]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    HotelItem *cell=[dataArray objectAtIndex:indexPath.row];
    HotelTableViewController *ce=[[HotelTableViewController alloc]init];
    ce.linkid=[cell.hotelid integerValue];
...
-(void)buttonMap:(UIButton *)button{
    ...
    HotelItem *item=[dataArray objectAtIndex:y];
    HotelTableViewController *ce=[[HotelTableViewController alloc]init];
    ce.linkid=[item.hotelid integerValue];

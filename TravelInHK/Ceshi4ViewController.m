...
#import "FMDatabase.h"
#import "Ceshi41ViewController.h"
#import "Ceshi42ViewController.h"
@interface Ceshi4ViewController ()

@end

@implementation Ceshi4ViewController
...
- (void)viewDidLoad
{
    ...
    FMResultSet *set=[db executeQuery:[NSString stringWithFormat:@"select * from listdo_shopping_main where scenicid='%d'",linkid]];
...
-(void)DataInorOut:(UITapGestureRecognizer *)tap{
    ...
    if(im.image==[UIImage imageNamed:@"favorite_botton_off"]){
        ...
        res=[db executeUpdate:[NSString stringWithFormat: @"INSERT INTO FVRT (id,title) VALUES (%d,'listdo_shopping_main')",linkid]];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    if(indexPath.section==0){
        if(indexPath.row==0){
            Ceshi41ViewController *ce=[[Ceshi41ViewController alloc]init];
            ce.scenicid=linkid;
    ...
    if(indexPath.section==1){
        Ceshi42ViewController *ce=[[Ceshi42ViewController alloc]init];
        ce.scenicid=linkid;
        [self.navigationController pushViewController:ce animated:YES];
        [ce release];
    }
